import time
from rstools.client import RSAgencySearchClient, RSItemClient, RSSeriesClient, RSSearchClient, RSSeriesSearchClient, UsageError, TooManyError
        ...
        while self.pages_complete < self.total_pages:
            ...
            for result in response['results']:
                ...
                for function in result['functions']:
                    if function['identifier'] == self.function.upper():
...
def clear_agencies():
    dbclient = MongoClient(MONGO_SERIES_URL)
    db = dbclient.get_default_database()
    db.functions.update_many({}, {'$unset': {'agencies': ''}})
